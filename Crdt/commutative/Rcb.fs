...
[<AutoOpen>]
module Crdt.Commutative.Protocol
...
and Protocol<'s,'c,'e> =
    ...
    | Replicated of from:ReplicaId * toSeqNr:uint64 * events:Commutative.Event<'e>[]
...
module ReplicationState =
    ...
    let unseen nodeId (state: ReplicationState<'s>) e =
...
let replicator (crdt: Crdt<'crdt,'state,'cmd,'event>) (db: Db) (id: ReplicaId) (ctx: Actor<Protocol<_,_,_>>) =    
    ...
    let rec active (db: Db) (state: ReplicationState<'crdt>) (replicatingNodes: Map<ReplicaId, ReplicationStatus<'crdt,'cmd,'event>>) (ctx: Actor<_>) = actor {
        ...
        | ReplicateTimeout nodeId ->
            ...
            let p = Map.tryFind nodeId replicatingNodes |> Option.defaultWith (fun () -> failwithf "didn't find '%s' among replicating nodes" nodeId)
